(
  ...
  "<dist> meters"
  [(dim :distance) #"(?i)metriä?"]
  (-> %1
     (dissoc  :latent)
     (merge {:unit "metre"}))

  "<dist> centimeters"
  [(dim :distance) #"(?i)cm|senttimetriä?|senttiä?"]
  (-> %1
     (dissoc  :latent)
  ...
  "<area> square meter"
  [(dim :area) #"(?i)neliömetriä?|neliö(ä|tä)?|m2"]
  (-> %1
     (dissoc :latent)
     (merge {:unit "square meter"}))
